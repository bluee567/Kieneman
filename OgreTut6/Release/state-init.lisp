...
(defclass+ state (low-y-box)
  ((:ia parent)
   ...
   (:ia state-buffer
	:initform nil)
   ;;Stores the state which is intented to be transfered to.
   (:ia key-buffer
...
(defclass+ clear-state-buffer ()
  ())

;; (defmacro get-buffered-state ()
;;   `(buffered-state parent))

;; (defmacro set-buffered-state (val)
;;   `(setf (buffered-state parent) ,val))

;; (defmethod exit-state :after (fighter (state1 clear-state-buffer))
;;   (setf (buffered-state state1) nil))
...
(defmacro capture-keymap-state ()
  `(with-accessors ((key-label key-label) (pressed pressed) (others-held others-held)) key-buffer
     (setf others-held (copy-tree (get-keymap)))))
;;NOTE: This will do alot of coppying, so replace with something else later.

(defmacro with-captured-keymap (&body body)
  `(with-accessors((others-held others-held)) key-buffer
    (with-keymap (if others-held others-held (get-keymap)) ;;Uses the captured keymap if it exists, otherwise the current keymap is used by default.
		 ,@body)))

(defmacro get-captured-keymap ()
  `(with-accessors((others-held others-held)) key-buffer
		  (if others-held others-held (get-keymap)) ;;Uses the captured keymap if it exists, otherwise the current keymap is used by default.
		  ))
...
(defmacro defstate (name &key slots rest supers constants funcs main-action initfunc entryfunc (type 'state) (hb 'multi-hitbox) (animation 'combined-linear-animation) (linear t) alt-name)
  (let* ((symbol-name (intern (string-upcase name)))
	 ...
	 (accessors (cons '(state-buffer state-buffer) (cons '(fighter parent) (mapcar #'(lambda (a) (list a a)) slot-names))))
