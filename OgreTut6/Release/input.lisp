...
(defun list-keys (&rest keys)
  (apply #'append
 	 (loop for (key fun name) on keys by #'cdddr collect (list key (create-input-key fun name)))))
...
(defun get-change (key &optional (keymap *keymap*))
  (let* ((raw-current-val (get-key-state key :current-state keymap))
	 (current-val (cond
		       ((numberp raw-current-val) raw-current-val)
		       ((not raw-current-val) 0.0)
		       (t 1.0)))
	 (raw-prev-val (get-key-state key :prev-state keymap))
	 (prev-val (cond
...
(defvar *k1-config*
  (list
   ...
   :dodge (get-kbi KC_I) "I"
   :cancel (get-kbi KC_O) "O"
   :throttle (get-kbi KC_M) "M"
   :down (get-kbi KC_S) "S"
   :up (get-kbi KC_W) "W"
